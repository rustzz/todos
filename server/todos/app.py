...
async def am_register(username: str = Query(None, min_length=4, max_length=50),
                      password: str = Query(None, min_length=64, max_length=64)):
    user = User((connect, config), username=username, password=password)
    return await user.register()


@app.post('/am/login')
async def am_login(username: str = Query(None, min_length=4, max_length=50),
                   password: str = Query(None, min_length=64, max_length=64)):
    user = User((connect, config), username=username, password=password)
...
async def notebook_add(username: str = Query(None, min_length=4, max_length=50),
...
async def notebook_get(username: str = Query(None, min_length=4, max_length=50),
                       token: str = Query(None, min_length=64, max_length=64)):
    user = User((connect, config), username=username, token=token)
    notes = Notes((connect, config), user)
...
async def am_check_token(username: str = Query(None, min_length=4, max_length=50),
                         token: str = Query(None, min_length=64, max_length=64)):
    user = User((connect, config), username=username, token=token)
...
async def am_check_user_exists(username: str = Query(None, min_length=4, max_length=50)):
